 ...
 * @return {EditorState}
 */
export const createExtendedRichTextEditor = (html = "", plugins = []) => {
  let doc, selection;
  
  if (html) {
    doc = createDocumentFromHtml(ExtendedrichTextSchema, html);
    selection = TextSelection.atEnd(doc);
  }
  
  return EditorState.create({
    schema: ExtendedrichTextSchema,
    doc,
    selection,
    plugins: [
      ...corePlugins,
 ...
 * @return {EditorState}
 */
export const createExtendedTwoRichTextEditor = (html = "", plugins = []) => {
  let doc, selection;
  
  if (html) {
    doc = createDocumentFromHtml(ExtendedTworichTextSchema, html);
    selection = TextSelection.atEnd(doc);
  }
